...
export async function chatOutput({
  ...
  if (!actor) return ui.notifications.warn("Aucun acteur disponible pour la sortie de chat.");
  ...
  const html = await foundry.applications.handlebars.renderTemplate("systems/mothership-fr/templates/qol/chat-output.html", {
    actor,
    title,
    subtitle,
    content,
